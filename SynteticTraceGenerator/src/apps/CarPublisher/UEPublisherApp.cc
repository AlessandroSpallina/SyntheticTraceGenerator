...
void UEPublisherApp::sendMessageToMECApp(){
    ...
    alert->setSensorName("TestSensor");
    alert->setUnit("TestUnit");
    alert->setValue(-0.9999);
    ...
    if(log)
    {
        ...
        if(myfile.is_open())
        {
            myfile <<"["<< NOW << "] UEPublisherApp - UE sent start subscription message to the MEC application \n";
            myfile.close();
        }
    }
