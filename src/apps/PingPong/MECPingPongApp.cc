...
float get_service_time(int lambda) {

    std::random_device rd;
    std::mt19937 gen(rd());

    std::exponential_distribution<> d(10-lambda);
...
void MECPingPongApp::initialize(int stage)
{
    ...
    auto filename = fmt::format("logs/mecpingpongapp_{}.txt", timestamp);

    static int counter = 0;
    myLogger = spdlog::basic_logger_mt(fmt::format("mecLogger{}", counter++), filename);

    numCars = par("numCars");

    myLogger->info(fmt::format("@ numCars {}", numCars));

    csv_filename = fmt::format("logs/mec_{}.csv", timestamp);
    ofstream myfile;
    myfile.open (csv_filename, ios::app);
    if(myfile.is_open())
    {
        myfile << "timestamp,vehicle_id,packet_size,e2e_delay,service_time" << endl;
        myfile.close();
    }
...
void MECPingPongApp::handleMessage(cMessage *msg)
{
    ...
    if (!msg->isSelfMessage())
    {
        if(!strcmp(msg->getName(), "PingPongPacket")) {
            ...
            if(myfile.is_open())
            {
                myfile << pk->getArrivalTime() << "," << ueAppAddress << "," << pk->getByteLength() << "," << simTime() - pk->getCreationTime() << "," << get_service_time(numCars) << endl;
                myfile.close();
            }
...
void MECPingPongApp::handleMp1Message()
{
...
void MECPingPongApp::handleServiceMessage()
{
