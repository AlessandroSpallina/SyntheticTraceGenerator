...
void MECPublisherApp::initialize(int stage)
{
    ...
    ofstream myfile;
    myfile.open ("mec_trace.csv", ios::app);
    if(myfile.is_open())
    {
        myfile << "timestamp,vehicle_id,sensor_name,sensor_value,packet_size" << endl;
        myfile.close();
    }
...
void MECPublisherApp::handleUeMessage(omnetpp::cMessage *msg)
{
    ...
    ofstream myfile;
        ...
        if(myfile.is_open())
        {
            myfile << pk->getArrivalTime() << "," << ueAppAddress << "," << sensorName << "," << sensorValue << "," << pk->getByteLength() << endl;
            myfile.close();
        }
}
